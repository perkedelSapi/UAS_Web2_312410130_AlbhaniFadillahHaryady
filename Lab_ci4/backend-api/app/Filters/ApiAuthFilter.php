    ...
    public function before(RequestInterface $request, $arguments = null)
    {
        ...
        if (!$token || empty($token)) {
            ...
            return $response->setJSON([
                'status'   => 401,
                'error'    => 401,
                'messages' => 'Sesi Token tidak valid atau expired'
            ]);
        }

        // 4. Validasi format token (harus mengandung prefix TOKEN-SECRET sesuai Api\Auth::login)
        $decoded = base64_decode($token);
        if (!str_starts_with($decoded, 'TOKEN-SECRET-')) {
            $response = Services::response();
            $response->setStatusCode(401);
            return $response->setJSON([
                'status'   => 401,
                'error'    => 401,
                'messages' => 'Token tidak sah.'
            ]);
        }
    }
